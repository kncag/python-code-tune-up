...
async function extractDNIsFromPdf(pdfBuffer: Uint8Array): Promise<Set<string>> {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Processing POST BCP-xlsx...');
    ...
    const dnis = await extractDNIsFromPdf(pdfBuffer);

    if (dnis.size === 0) {
      throw new Error('No se detectaron identificadores en el PDF');
    }
    ...
    // Filtrar filas que contengan alguno de los DNIs detectados
    const matchingRows = data.filter((row: any[]) => {
      return row.some(cell => dnis.has(String(cell)));
    });
    ...
    console.error('Error processing POST BCP-xlsx:', error);
