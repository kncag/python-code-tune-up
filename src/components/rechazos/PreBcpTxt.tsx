...
const PreBcpTxt = () => {
  const [code, setCode] = useState("R002");
  const [desc, setDesc] = useState("CUENTA INVALIDA");
  ...
  const handleProcess = async () => {
    if (!pdfFile || !txtFile) {
      toast({
        ...
        description: "Debe seleccionar ambos archivos (PDF y TXT)",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Button
        ...
        disabled={!pdfFile || !txtFile || loading}
        className="w-full"
        size="lg"
      >
        {loading ? (
